...
function Gate() {
  ...
  return <>
    <AppShellProvider><ApiKeysProvider><AppLayout><PageTransition><CurrentPage /></PageTransition></AppLayout></ApiKeysProvider></AppShellProvider>
...
function AiNoticeModal({ onClose }: { onClose: () => void }) {
  return (
    <div role="presentation" onMouseDown={onClose} className="modal-overlay" style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, background: 'rgba(27,26,23,.38)', backdropFilter: 'blur(5px)' }}>
      <section role="dialog" aria-modal="true" aria-labelledby="ai-notice-title" onMouseDown={(event) => event.stopPropagation()} className="modal-panel" style={{ width: 'min(100%, 500px)', borderRadius: 24, padding: '28px 28px 24px', background: '#fffdf8', boxShadow: '0 24px 70px rgba(40,30,20,.25)', color: '#1b1a17' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
          ...
          <button type="button" onClick={onClose} aria-label="关闭提示" className="btn-press" style={{ border: 'none', background: 'transparent', color: '#8a8478', fontSize: 24, lineHeight: 1, cursor: 'pointer' }}>×</button>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <a href="https://github.com/SugarVei/sugar-job-system" target="_blank" rel="noreferrer" className="btn-press" style={{ flex: 1, minWidth: 180, height: 44, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', borderRadius: 999, background: '#1b1a17', color: '#fffdf8', textDecoration: 'none', fontSize: 14, fontWeight: 600 }}>去 GitHub 点 Star ↗</a>
          <button type="button" onClick={onClose} className="btn-press" style={{ height: 44, padding: '0 22px', border: '1.5px solid #e4ddcf', borderRadius: 999, background: '#fffdf8', color: '#1b1a17', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>知道了</button>
        </div>
      </section>
    </div>
  );
