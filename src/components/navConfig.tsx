...
import { IconDashboard, IconOverview, IconApplications, IconCapitalMap, IconCompanies, IconResumes, IconInterviews, IconTrophy, IconKey, IconOffer, IconMail, IconPlugin } from './icons';
...
export const NAV_ITEMS: { key: ScreenKey; label: string; shortLabel?: string; group: NavGroupId; mobilePrimary?: boolean; Icon: typeof IconDashboard }[] = [
  ...
  { key: 'capitalMap', label: '省会校招', shortLabel: '地图', group: 'talent', Icon: IconCapitalMap },
  ...
  { key: 'interviews', label: '面试日历与面经', shortLabel: '面试', group: 'career', mobilePrimary: true, Icon: IconInterviews },
...
export function greetFor(screen: ScreenKey, name: string): { title: string; sub: string } {
  ...
  const map: Record<ScreenKey, { title: string; sub: string }> = {
    dashboard: { title: `${timeGreeting()}，${who}`, sub: '今天有几件事临近截止，先把它们处理掉' }, overview: { title: '投递总览', sub: '投递的城市、薪资、岗位与渠道，一屏看全' }, applications: { title: '投递记录', sub: '记录每次投递，按公司、状态、渠道筛选' }, capitalMap: { title: '省会校招', sub: '点省会看当地龙头企业校招网址' }, companies: { title: '公司库', sub: '自动汇总已投递公司，关联岗位与进度' }, referralCodes: { title: '内推码管理', sub: '集中保存公司、内推码与推荐人信息' }, hotCompanies: { title: '热门公司', sub: '精选知名企业，搜索与管理你添加的公司' }, resumes: { title: '简历库', sub: '多版本简历与面试稿件，按账号保存' }, interviews: { title: '面试日历与面经', sub: '安排面试，也把每次经验沉淀下来' }, offers: { title: 'Offer 管理', sub: '统一比较薪资、截止时间与决策风险' }, interviewReviews: { title: '智能填表助手', sub: '网站管理资料，插件负责填写' }, resumeAssistant: { title: '智能填表助手', sub: '网站管理资料，插件负责填写' }, mailbox: { title: '面试邮件', sub: '连接网易 163，在系统内查看面试相关邮件' },
  };
  return map[screen];
}
