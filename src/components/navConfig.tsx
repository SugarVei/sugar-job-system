...
export const NAV_ITEMS: {
  ...
  { key: 'interviewReviews', label: '面试复盘', shortLabel: '复盘', group: 'career', Icon: IconReview },
  { key: 'mailbox', label: '面试邮件', shortLabel: '邮件', group: 'career', Icon: IconMail },
];

export const NAV_GROUPS: { id: NavGroupId; label: string }[] = [
  { id: 'core', label: '求职主线' },
  { id: 'talent', label: '公司与简历' },
  { id: 'career', label: '面试与决策' },
];
...
export function greetFor(screen: ScreenKey, name: string): { title: string; sub: string } {
  ...
  const map: Record<ScreenKey, { title: string; sub: string }> = {
    dashboard: { title: `${timeGreeting()}，${who}`, sub: '今天有几件事临近截止，先把它们处理掉' },
    ...
    hotCompanies: { title: '热门公司', sub: '精选知名企业，搜索与管理你添加的公司' },
    ...
    mailbox: { title: '面试邮件', sub: '连接网易 163，在系统内查看面试相关邮件' },
  };
  return map[screen];
}
