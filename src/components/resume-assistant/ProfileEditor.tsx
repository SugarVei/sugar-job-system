...
export function ProfileEditor({ profile, setProfile, onSave, saving, localOnly, credential }: Props) {
  ...
  const analyze = async () => {
    ...
    setAnalyzing(true); setError(''); setAnalysis(null); setAnalysisApplied(false);
    try {
      const resumeText = await extractText(resumeFile);
      const result = await resumeAssistantApi.analyzeResumeProfile(resumeText);
