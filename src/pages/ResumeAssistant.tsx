...
import { SyncPreview } from '../components/resume-assistant/SyncPreview';
...
export default function ResumeAssistant() {
  ...
  return <div style={{ display: 'grid', gap: 16, paddingBottom: 26 }}>
    ...
    {profile.localOnly && <div style={{ ...creamCard, borderColor: '#ecd29b', background: '#fff8e8', ...muted }}>当前是本机演示/草稿模式：资料保存在当前浏览器，待配置 Supabase 和 API 后可同步到插件。</div>}
    ...
    {assistantTab === 'profile' && <div style={{ display: 'grid', gap: 16 }}><ProfileEditor profile={profile.profile} setProfile={profile.setProfile} onSave={() => void profile.save()} saving={profile.saving} localOnly={profile.localOnly} /><SyncPreview profile={profile.preview} scope={profile.syncScope} /></div>}
