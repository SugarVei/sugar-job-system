...
export default function JdMatches(){
  ...
  const analyze=async()=>{
    ...
    try{const row=await matches.create(payload);setCurrent(row);setLastSavedAt(new Date());setState('result')}catch(e){setError(`生成失败，未保存：${e instanceof Error?e.message:String(e)}`);setState('input')}
  };
  ...
  return <div className="flex flex-col gap-[18px] animate-rise">
    <div className="jd-actionbar" style={{...CARD,borderRadius:18}}><div className="flex flex-wrap gap-2"><GhostButton onClick={()=>setState('history')} style={{minWidth:112}}>历史分析 ({matches.items.length})</GhostButton><PrimaryButton accent={theme.accent} onClick={fresh} style={{minWidth:104}}>＋ 新建分析</PrimaryButton></div><span className="text-xs text-[#8a8478] self-center">无 AI Key 也可用 · 生成后自动保存到账号</span></div>
