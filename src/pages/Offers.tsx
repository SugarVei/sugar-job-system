...
export default function Offers() {
  ...
  return <div className="flex flex-col gap-[18px] animate-rise">
    ...
    <div className="grid grid-cols-2 sm:flex gap-3 p-3" style={{ ...CARD, borderRadius: 18 }}>
      <div className="flex rounded-xl bg-[#f5f0e7] p-1">
      ...
      <Select value={status} onChange={e => setStatus(e.target.value)}><option>全部状态</option>{statuses.map(x => <option key={x}>{x}</option>)}</Select>
      <Select value={sort} onChange={e => setSort(e.target.value)}><option value="reply_deadline">按回复截止</option><option value="annual_package">按年包</option><option value="total_score">按综合评分</option></Select>
      <PrimaryButton accent={theme.accent} onClick={openCreate}>＋ 新增 Offer</PrimaryButton>
    </div>
    ...
    <Modal open={modal} title={editing ? '编辑 Offer' : '新增 Offer'} onClose={() => setModal(false)} footer={<><GhostButton onClick={() => setModal(false)}>取消</GhostButton><PrimaryButton accent={theme.accent} onClick={() => void save()} disabled={saving}>{saving ? '保存中…' : '保存'}</PrimaryButton></>}>
      <FormError message={formError} />
      <h3>基本信息</h3><Field label="关联投递记录"><Select value={form.application_id ?? ''} onChange={e => selectApplication(e.target.value)}><option value="">手动录入</option>{apps.items.filter(x => x.status === 'Offer').map(x => <option value={x.id} key={x.id}>{x.company_name} · {x.position_name}</option>)}</Select></Field>
      <div className="grid sm:grid-cols-2 gap-x-3"><Field label="公司名称 *"><TextInput value={form.company_name} onChange={e => patch('company_name',e.target.value)} /></Field><Field label="岗位名称 *"><TextInput value={form.position_name} onChange={e => patch('position_name',e.target.value)} /></Field><Field label="城市"><TextInput value={form.city ?? ''} onChange={e => patch('city',e.target.value)} /></Field><Field label="状态"><Select value={form.offer_status} onChange={e => patch('offer_status',e.target.value as OfferStatus)}>{statuses.map(x => <option key={x}>{x}</option>)}</Select></Field><Field label="回复截止"><TextInput type="datetime-local" value={form.reply_deadline?.slice(0,16) ?? ''} onChange={e => patch('reply_deadline', e.target.value || null)} /></Field><Field label="入职日期"><TextInput type="date" value={form.join_date ?? ''} onChange={e => patch('join_date',e.target.value || null)} /></Field></div>
      <h3>薪资结构（元）</h3><div className="grid grid-cols-2 gap-x-3">{(['base_salary','salary_months','bonus','subsidy','annual_package'] as const).map((k,i) => <Field key={k} label={['月薪','薪数','奖金','年度补贴','年包（可手填）'][i]}><TextInput type="number" value={form[k] ?? ''} onChange={e => patch(k,n(e.target.value))} /></Field>)}</div>
      <h3>评分（0–100）</h3><div className="grid grid-cols-2 gap-x-3">{(['salary_score','match_score','growth_score','stability_score','city_score','workload_score'] as const).map((k,i) => <Field key={k} label={['薪资','匹配','成长','稳定','城市','工作量'][i]}><TextInput type="number" min="0" max="100" value={form[k] ?? ''} onChange={e => patch(k,n(e.target.value))} /></Field>)}</div>
      <h3>谈薪与风险</h3><Field label="谈薪记录"><TextArea value={form.negotiation_notes ?? ''} onChange={e => patch('negotiation_notes',e.target.value)} /></Field>
      <div className="flex flex-wrap gap-3 mb-4">{(['is_big_week','is_overtime','is_remote','probation_cut','has_penalty'] as const).map((k,i) => <label key={k}><input type="checkbox" checked={form[k]} onChange={e => patch(k,e.target.checked)} /> {['大小周','加班','可远程','试用期折薪','违约金'][i]}</label>)}</div>
      <Field label="风险备注"><TextArea value={form.risk_notes ?? ''} onChange={e => patch('risk_notes',e.target.value)} /></Field><Field label="备注"><TextArea value={form.notes ?? ''} onChange={e => patch('notes',e.target.value)} /></Field>
    </Modal>
  </div>;
}
...
const tab = (active:boolean):React.CSSProperties => ({ minHeight:36,padding:'0 14px',border:0,borderRadius:9,background:active?'#fffdf8':'transparent',fontWeight:700 });
