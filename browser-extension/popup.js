...
document.querySelector('#fill').onclick = async () => { const [tab] = await chrome.tabs.query({ active: true, currentWindow: true }); const r = await chrome.tabs.sendMessage(tab.id, { type: 'fill' }); result(r?.fields_filled != null ? `已填写 ${r.fields_filled}/${r.fields_total} 项；不会提交表单。` : '当前页面无法填写。'); };
status();
