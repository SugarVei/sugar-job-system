...
async function loadResumeProfile() {
  ...
  isResumeDirty = false;
  // Keep manual saving available even when no DOM input/change event fired.
  // Browser autofill and programmatic value updates do not always emit those
  // events, which previously left the save button permanently disabled.
...
async function persistResumeProfile({ silent = false } = {}) {
  ...
  try {
    ...
    console.error("[resume-editor] 保存标准简历失败:", error);
    updatePageStatus("error", `保存失败：${error?.message || "扩展存储不可用"}`);
    throw error;
  } finally {
    saveResumeBtn.disabled = false;
    saveResumeBtn.textContent = "保存标准简历";
  }
}
...
saveResumeBtn.addEventListener("click", async () => {
  try {
    ...
    // persistResumeProfile already reports the actionable error on the page.
  }
});
