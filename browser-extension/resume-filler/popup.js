...
const editProviderInput = document.getElementById("editProvider");
const providerPresets = globalThis.ResumeProviderPresets;
for (const preset of providerPresets.presets) {
  editProviderInput.add(new Option(preset.name, preset.id));
}
editProviderInput.addEventListener("change", () => {
...
function openEditModal(modelId = null) {
  ...
  if (modelId) {
    editModalTitle.textContent = "配置 API";
    loadModelForEdit(modelId);
    return;
  }

  editModalTitle.textContent = "添加 API 厂商";
  editProviderInput.value = "deepseek";
  editApiKeyInput.value = "";
}
      ...
      (model) => `
        <div class="model-item ${model.id === activeId ? "active" : ""}" data-model-id="${escapeHtml(model.id)}">
          ...
          <div class="model-info">
            ...
            <div class="model-meta">${model.apiKey ? "已配置密钥 · 可使用" : "尚未配置 · 请点击设置"}</div>
          </div>
          <div class="model-actions">
              <button aria-label="配置 API" class="icon-btn edit-model-btn" data-model-id="${escapeHtml(model.id)}">
...
async function loadModelForEdit(modelId) {
  ...
  const preset = providerPresets.match(model);
  editProviderInput.value = preset?.id || "deepseek";
  editApiKeyInput.value = preset ? model.apiKey : "";
  if (!preset) showEditStatus("error", "旧自定义接口不在预设列表中，请选择厂商并填写对应密钥。");
}

saveModelBtn.addEventListener("click", async () => {
  const preset = providerPresets.presets.find(p => p.id === editProviderInput.value);
  if (!preset) return;
  const { name, baseUrl, model } = preset;
  const apiKey = editApiKeyInput.value.trim();
  let savedId = editingModelId;

  if (!name || !baseUrl || !apiKey || !model) {
    showEditStatus("error", "请输入所选厂商的 API Key");
    return;
  }
  ...
  try {
    ...
    if (editingModelId === BUILTIN_MODEL.id) {
      ...
      savedId = `provider-${preset.id}-${Date.now()}`;
      models.push({
        id: savedId,
        name,
        baseUrl,
    ...
    await modelStorage.saveActiveModelId(savedId);
    showEditStatus("success", "已保存并启用");
    setTimeout(() => {
      closeEditModal();
      renderModelList();
    }, 300);
