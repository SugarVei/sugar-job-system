...
importScripts("shared/model-storage.js", "shared/provider-presets.js");
...
async function callAI(modelId, prompt, mode) {
  ...
  if (!baseUrl || !apiKey || !model) {
    throw new Error("请先在 API 设置中选择厂商并保存自己的 API Key");
  }
  ...
  try {
    response = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        temperature: 0.2,
        ...globalThis.ResumeProviderPresets.requestOptions(config),
        messages: [
