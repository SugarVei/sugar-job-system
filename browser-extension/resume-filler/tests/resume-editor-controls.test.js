...
test("resume editor keeps manual save available after loading and importing", () => {
  const source = fs.readFileSync(
    path.join(__dirname, "../resume-editor.js"),
    "utf8"
  );

  const loadProfile = extractFunction(
    source,
    "async function loadResumeProfile() {",
    "function renderResumeEditor(profile) {"
  );
  assert.match(loadProfile, /saveResumeBtn\.disabled = false;/);

  const importHandler = source.slice(source.indexOf('importResumeBtn.addEventListener("click"'));
  assert.match(importHandler, /saveResumeBtn\.disabled = false;/);
});
